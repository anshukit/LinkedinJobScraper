...
#         try:
#             await page.goto(search_url, timeout=80000, wait_until="networkidle")
#             break
#         except TimeoutError:
#             if attempt == retries:
...
#     for attempt in range(1, retries + 1):
#         try:
#             await page.wait_for_selector("div.feed-shared-update-v2", timeout=40000)
#             break
#         except TimeoutError:
#             if attempt == retries:
...
async def scrape_all_keywords_parallel(keywords: list[str], posts_per_keyword: int, max_concurrent_tabs, task_id=None):
    total_posts = posts_per_keyword * len(keywords)
    progress_bar = tqdm(total=total_posts, desc="Total Scraping Progress", ncols=80)
